...
public class guiPatientLoginController {
    public TextField cprField;
    public TextField nameField;
    public Button createPatient;

    public void loginPatientButton2(ActionEvent event) throws IOException, SQLException {
        Login login = new Login();
        boolean succes = login.patientLogin(nameField.getText(), cprField.getText());
        if (!succes) {
            System.out.println("ERROR");
        } else {
            Parent loginPatient = FXMLLoader.load(getClass().getResource("guiPatientData.fxml"));
